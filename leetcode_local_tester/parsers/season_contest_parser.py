...
from leetcode_local_tester.parsers.utils import parse_season_sample
...
class SeasonContestParser(Parser):
    ...
    def get_basic_info(self, session: Session, problem: Problem) -> (str, str, bool, List[Function]):
        ...
        if default_code == "":
            raise Exception(f"Cannot find code definition for {self.creator.code_type}")
    ...
    def get_sample(self, session: Session, problem: Problem) -> (List[List[str]], List[List[str]]):
        ...
        node = self._get_node(session, problem.url)
        return parse_season_sample(node, is_func_problem)
